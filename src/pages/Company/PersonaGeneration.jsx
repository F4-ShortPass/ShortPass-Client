...
const PersonaGeneration = () => {
  ...
  return (
    <div className="w-full min-h-screen flex justify-center my-15">
      <div className="w-full max-w-5xl flex flex-col mx-auto gap-10 px-4 md:px-6">
        {/* Header */}
        <div className="text-center">
          <h1 className="text-3xl font-bold mb-10 text-blue">면접 페르소나 생성하기</h1>
          <div className="flex justify-center gap-2.5 md:gap-8 md:mb-8 text-sm md:text-base  whitespace-nowrap">
            <div className={`px-4 py-2 rounded-full ${step >= 1 ? 'bg-blue-600 text-white font-medium' : 'bg-gray-100'}`}>
              1. JD 업로드
            </div>
            <div className={`px-4 py-2 rounded-full ${step >= 2 ? 'bg-blue-600 text-white font-medium' : 'bg-gray-100'}`}>
              2. 역량 확인
            </div>
            <div className={`px-4 py-2 rounded-full ${step >= 3 ? 'bg-blue-600 text-white font-medium' : 'bg-gray-100'}`}>
              3. 페르소나 생성
            </div>
          </div>
        </div>
        ...
        {step === 1 && (
          <div className="flex flex-col gap-6">
            <div className="bg-white p-6 rounded-lg border border-gray-300">
              ...
              <PdfUpload
                ...
                onFileChange={(file) => {
                  ...
                  setUploadStatus(file ? `${file.name}` : '');
                }}
            ...
            <div className="flex items-center justify-between">
              <Button onClick={() => navigate('/')} disabled={loading} variant='ghost'>
                이전
              </Button>
        ...
        {step === 2 && (
          <div className="flex flex-col gap-6">
            <div className="bg-white p-6 rounded-lg border border-gray-200">
              <h2 className="text-lg md:text-xl font-semibold mb-1 text-blue">직무 역량 확인하기</h2>
              ...
              <div className='flex flex-wrap gap-3 mb-8'>
                {['데이터 분석', "고객 여정 마케팅", "시즌 전략 KPI", "가치사슬 최적합", "이해관계자 협업"].map((competency) => (<Badge variant='plain' classname='px-2.5 py-1 whitespace-nowrap rounded-lg bg-blue-50 text-blue-600 text-sm md:text-base font-medium' >{competency}</Badge>)) }
              </div>

              <div className="mb-8">
                <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-4 md:mb-2">
                  <h3 className="text-base md:text-xl font-semibold text-blue">가중치 설정하기</h3>
            ...
            <div className="flex items-center justify-between">
              ...
              <Button onClick={handleGeneratePersona} variant='primary' disabled={loading}>
        ...
        {step === 3 && (
          <div className="flex flex-col gap-6">
                  ...
                  <div className="grid gap-5">
                    {generatedPersonas.map((persona, idx) => {
                      ...
                      return (
                        <div
                          key={persona.id}
                          className="rounded-xl p-5 h-full border border-gray-300"
                        >
                          <div className="flex items-start justify-between gap-3">
                            <div>
                              <h4 className="text-xl font-bold text-gray-900 leading-snug whitespace-nowrap">{persona.persona_name}</h4>
                              <p className="text-base text-gray-800 mt-2 leading-snug whitespace-nowrap">
                                {summaryLine || '질문 포커스를 확인하세요'}
                              </p>
                            </div>
                            {persona.tone && (
                              <p className="text-sm px-3 py-1 text-blue-700 font-semibold">
                                {persona.tone}
                              </p>
                            )}
                        ...
                        {persona.focus_keywords?.length > 0 && (
                          <>
                          <p className='text-base font-medium'>JD 추출 키워드 :</p>
                            <div className='flex flex-wrap gap-3'>
                              {persona.focus_keywords.map((keyword) => (<Badge variant='tertiary'>{keyword}</Badge>))}
                            </div>
                          </>
                        )}
                        ...
                        <div className="mt-4">
                          <div className="flex items-center justify-between mb-2">
                            <p className="text-base font-semibold">필수 질문</p>
            ...
            <div className="flex justify-between gap-4">
              <Button onClick={() => setStep(2)} disabled={loading} variant='ghost'>
                역량 재확인
              </Button>
              <Button
                onClick={() =>

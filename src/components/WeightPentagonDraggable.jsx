  ...
  return (
    <div className='w-full flex flex-col gap-4'>
      <div className='text-sm text-gray-700'>
        {/* 안내 문구 업데이트 */}
        오각형 꼭짓점을 <strong className='text-blue'>드래그</strong>해 가중치를 조정하세요. 합은 자동으로 <strong className='text-blue'>1</strong>로 정규화됩니다.
      </div>
      <div className='w-full flex flex-col gap-6 items-center mt-5'>
        ...
        <div className='flex-1 grid grid-cols-1 sm:grid-cols-2 gap-3 w-full'>
          {localWeights.map((w, idx) => (
            <div key={w.id || idx} className='p-4 rounded-lg border bg-white border-gray-200'>
              <div className='flex justify-between items-center mb-2'>
                <div className='font-semibold text-gray-900'>{w.label || w.id}</div>
                {/* 100%가 넘을 수 있으므로 파란색 강조 */}
                <div className={`font-semibold ${w.value > 1 ? 'text-red-600' : 'text-blue-700'}`}>
